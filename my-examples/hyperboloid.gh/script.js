...
async function onSolve(){
  ...
  let res = await fetch('/solve', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload), signal: currentSolveAbort.signal }).catch(e=>{ if (e?.name === 'AbortError') return null; return { ok:false, text: ()=> Promise.resolve(String(e&&e.message||'network error')) } })
...
function renderFallbackAll(inputs){
  ...
  try{
    ...
    if (scenes[0] && scenes[0].group){
      ...
      vB.group.add(clone)
      vB.group.traverse(o=>{ if (o.isMesh && o.material){ o.material.color = new THREE.Color(viewers[1].color) } })
    }
  ...
  try{
    ...
    if (scenes[0] && scenes[0].group){
      const clone = scenes[0].group.clone(true)
      vC.group.add(clone)
      vC.group.traverse(o=>{ if (o.isMesh && o.material){ o.material.color = new THREE.Color(viewers[2].color) } })
    }
