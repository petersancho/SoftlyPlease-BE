...
echo "== Solve =="
curl -sS -X POST "$H/solve/" -H "Content-Type: application/json" -d '{"definition":"BranchNodeRnd.gh","inputs":{"Count":5,"Radius":3,"Length":2}}' | head -c 400 || true

echo "== Status =="
curl -sS "$H/status" || echo "[Status endpoint down]"
